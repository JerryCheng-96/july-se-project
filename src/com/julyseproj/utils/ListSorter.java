...
//Source:https://blog.csdn.net/weixin_41888013/article/details/80921996
//Author:yinaibang
...
public class ListSorter {
            ...
            public int compare(E a, E b) {
                ...
                try {
                    for (int i = 0; i < sortnameArr.length; i++) {
                        ret = ListSorter.compareObject(sortnameArr[i], isAsc, a, b);
                        if (0 != ret) {
                            break;
                        }
                    }
    ...
    public static <E> void sort(List<E> list, final String[] sortnameArr, final boolean[] typeArr) {
        ...
        Collections.sort(list, new Comparator<E>() {
            public int compare(E a, E b) {
                ...
                try {
                    for (int i = 0; i < sortnameArr.length; i++) {
                        ret = ListSorter.compareObject(sortnameArr[i], typeArr[i], a, b);
                        if (0 != ret) {
                            break;
                        }
                    }
    ...
    private static <E> int compareObject(final String sortname, final boolean isAsc, E a, E b) throws Exception {
        int ret;
        Object value1 = ListSorter.forceGetFieldValue(a, sortname);
        Object value2 = ListSorter.forceGetFieldValue(b, sortname);
        String str1 = value1.toString();
        String str2 = value2.toString();
        if (value1 instanceof Number && value2 instanceof Number) {
            int maxlen = Math.max(str1.length(), str2.length());
            str1 = ListSorter.addZero2Str((Number) value1, maxlen);
            str2 = ListSorter.addZero2Str((Number) value2, maxlen);
        } else if (value1 instanceof Date && value2 instanceof Date) {
            long time1 = ((Date) value1).getTime();
            long time2 = ((Date) value2).getTime();
            int maxlen = Long.toString(Math.max(time1, time2)).length();
            str1 = ListSorter.addZero2Str(time1, maxlen);
            str2 = ListSorter.addZero2Str(time2, maxlen);
        }
        if (isAsc) {
